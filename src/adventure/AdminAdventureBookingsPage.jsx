...
const AdminAdventureBookingsPage = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const res = await api.get("/api/adventures/orders/admin/all-bookings", {
        params,
      });
  ...
  if (!user || user.role !== "admin") {
    return (
      <div className='p-6 text-red-600'>
        Only admins can view all adventure bookings.
      </div>
    );
  }
          ...
          <tbody>
            {bookings.length === 0 ? (
              ...
              bookings.map((b) => (
                <tr
                  key={b._id}
                  className='bg-blue-100 hover:bg-blue-200 rounded'
                >
        ...
        {Array.from({ length: totalPages }, (_, i) => (
          <button
            ...
            className={`px-3 py-1 rounded ${
              filters.page === i + 1 ? "bg-blue-900 text-white" : "bg-gray-200"
            }`}
